...
This guideline is for agents reviewing and rewriting GitHub Release notes for npm-fw.

## Background

Release notes are initially generated automatically from commit logs by `changelogithub --draft`. Treat them as a draft, not as the final source of truth.
...
- Internal implementation details may be included even when they are not useful to npm-fw users.
...
Review the draft from the perspective of the released npm-fw software, not from the perspective of the repository commit history.

Ask:

- What changed for users or operators of npm-fw?
...
Use `Features` for new or expanded npm-fw capabilities visible to users, operators, or API consumers.

Do not classify development workflow improvements as product features. For example, adding lefthook-based commit checks may be a `feat` commit, but it is not an npm-fw software feature and usually should not appear in release notes.
...
Use `Performance` only for changes that improve runtime behavior, responsiveness, resource usage, or scalability of npm-fw.
...
[View changes on GitHub](https://github.com/d-kimuson/npm-fw/compare/vPREVIOUS...vNEXT)
```
